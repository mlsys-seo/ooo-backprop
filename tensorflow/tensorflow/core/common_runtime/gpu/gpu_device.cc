...
Status BaseGPUDevice::Init(const SessionOptions& options) {
  ...
  sub_stream_device_context_ =
      new GPUDeviceContext(1, sub_stream_->compute,
#if TENSORFLOW_USE_ROCM
                           stream_->nccl,
#endif
                           stream_->host_to_device, stream_->device_to_host,
  ...
  if (do_ooo_backprop_) {
    const char* cstr_capture_op_name = std::getenv("OOO_CAPTURE_OP");
    capture_op_name_ = std::string(cstr_capture_op_name ? cstr_capture_op_name : "");
...
void BaseGPUDevice::Compute(OpKernel* op_kernel, OpKernelContext* context) {
  ...
  ScopedMemoryDebugAnnotation op_annotation(op_kernel->name_view().data(),
                                            context->step_id());
  bool is_capture_phase = (!capture_op_name_.empty() &&
