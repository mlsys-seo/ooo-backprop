...
def parse_args():
    ...
    delattr(args_for_print, 'async_op')
    delattr(args_for_print, 'prefix')
    delattr(args_for_print, 'separater')
    delattr(args_for_print, 'num_dependency')

    _print_args(args_for_print, unknown_args)
    
    return args

def _add_OOO_args(parser):
    group = parser.add_argument_group(title='OOO args')
    group.add_argument('--reverse_first_k', type=int, default=45,
...
def _add_debugging_args(parser):
    group = parser.add_argument_group(title='training args')
    group.add_argument('--debug_print', type=str_to_bool, default=False,
