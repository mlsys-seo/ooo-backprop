...
template <typename T>
struct CudaGraphRun<CPUDevice, T> { 
  void operator()(const CPUDevice& d) { 
    std::cout << "CudaGraphRun functor cpu" << std::endl;
  }
};
...
template <typename Device, typename T>
class PrepareCudaGraphCaptureOp: public OpKernel {
 private:

 public:
  explicit PrepareCudaGraphCaptureOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
  }
...
#define REGISTER_KERNELS(D, T)                                                \
  REGISTER_KERNEL_BUILDER(                                                    \
      Name("CudaGraphRun").Device(DEVICE_##D).TypeConstraint<T>("T"), \
      PrepareCudaGraphCaptureOp<D##Device, T>);                                  
#define REGISTER_CPU_KERNELS(T) REGISTER_KERNELS(CPU, T);
...
#define DECLARE_GPU_SPEC(T)                             \
  template <>                                           \
  void CudaGraphRun<GPUDevice, T>::operator()(  \
      const GPUDevice& d);             \
  extern template struct CudaGraphRun<GPUDevice, T>;
DECLARE_GPU_SPEC(Eigen::half);
DECLARE_GPU_SPEC(float);
DECLARE_GPU_SPEC(double);
#undef DECLARE_GPU_SPEC
