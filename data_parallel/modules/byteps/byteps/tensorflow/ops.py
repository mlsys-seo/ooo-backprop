...
'''Customize by OOO_backprop
'''

import os
DEBUG_C_PRINT = bool(os.getenv("DEBUG_C_PRINT"))

def _polling_data(DUMMY_PLACEHOLDER, op_index, s0, s1, s2, s3):
    return C_LIB.polling_data(
        DUMMY_PLACEHOLDER,
        op_index=op_index,
        s0=s0,
        s1=s1,
...
def _comm_terminal(input_tensor):
    return C_LIB.comm_terminal(input_tensor)
...
def _push_pull(tensor, scope='', name=None):
    ...
    if "LATER" in tensor.name:
        op_index = get_op_index(tensor.name)
        if DEBUG_C_PRINT:
            print(f"[OOO-LOG:: PYTHON] Call ASYNC | index: {op_index} name: {name}")
        return C_LIB.byteps_push_pull_async(tensor, name=name, input_name = full_name, op_index=op_index, debug_name = tensor.name )
    
    else:
        if DEBUG_C_PRINT:
            op_index = get_op_index(tensor.name)
            print(f"[OOO-LOG:: PYTHON] Call SYNC | index: {op_index} name: {name}")
        return C_LIB.byteps_push_pull(tensor, name=name, input_name = full_name)

def get_op_index(name):
    PREFIX = "LAYER_"
    SEPARATOR = "_END"
    base = name.find(PREFIX)
    offset = name.find(SEPARATOR)

    if( base == -1 or offset == -1 ):
        print( "[PYTHON-ERROR] get_op_index can't get index ", name )
        exit()
