...
void ThunkSchedule::AddDependenciesOnTransitiveOperands(
  ...
  if (hlo_to_thunk.contains(&operand)) {
    ...
    if (stream_assignment_->StreamNumberForHlo(operand) !=
      ...
      // std::cout << "depends_on_ - thunk : " << thunk_to_hlo_.at(&thunk)->metadata().op_name()
      //           << ", operand : " << operand.metadata().op_name() << std::endl;
...
bool OverlapForward(const HloInstruction* hlo) {
  ...
  return (op_name.find("Conv2DBackpropFilter") != std::string::npos &&
          op_name.find("Dummy") == std::string::npos &&
          op_name.find("FWD") != std::string::npos);
}
...
bool IsDummyOp(const HloInstruction* hlo) {
  ...
  return op_name.find("DummyConv2DBackpropFilter") != std::string::npos;
}

void ThunkSchedule::RemoveDummyOpDependencyEdges() {
  for (const Thunk* dst : thunk_total_order_) {
    if (!depends_on_.contains(dst)) {
      continue;
    }

    if (GetThunkOpName(dst).find("XLA_Retvals") == std::string::npos) {
      continue;
    }
    ...
    for (auto iter = sources.begin(); iter != sources.end();) {
      ...
      if (IsDummyOp(thunk_to_hlo_.at(src))) {
        iter = sources.erase(iter);
      } else {
        ++iter;  
      }
